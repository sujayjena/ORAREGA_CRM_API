...
namespace OraRegaAV.Models
{
    public class Utilities
    {
        ...
        public static string InvoiceNumberAutoGenerated_For_Maharashtra()
        {
            var quotationNo = "MHI0000001";
            using (var db = new dbOraRegaEntities())
            {
                var obj = db.tblQuotations.OrderByDescending(x => x.Id).FirstOrDefault();
                if (obj != null)
        ...
        public static string InvoiceNumberAutoGenerated_For_GOA()
        {
            var quotationNo = "GAI0000001";
            using (var db = new dbOraRegaEntities())
            {
                var obj = db.tblQuotations.OrderByDescending(x => x.Id).FirstOrDefault();
                if (obj != null)
                {
                    var QuotationNumber = obj.QuotationNumber;
                    quotationNo = "GAI" + (Convert.ToInt32(QuotationNumber.Substring(7, QuotationNumber.Length - 7)) + 1).ToString("D7");
                }
            }
            return quotationNo;
        }
